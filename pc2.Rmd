...
gener_tabla$es_relativa <- grepl("^/|#", gener_tabla$url, perl = T)
gener_tabla$url_absoluta <- gener_tabla %>% mutate(url_absoluta = ifelse(es_relativa, paste("https://www.mediawiki.org", url), url))
gener_tabla <- gener_tabla %>% mutate(url_absoluta = ifelse(es_relativa, paste0("https://www.mediawiki.org", url), url))


get_status_code <- function(my_url) {
  response <- GET(my_url)
